...
public class SortingAndSearch  <T extends Comparable {
    ...
    public boolean binarysearch (T[]data, int min, int max, T target){
      //-1 Cuando el valor es menos, 0=cuando el valor es igual, 1=Cuando el valor es mayor
      boolean found=false;
      int mindpoint=(min+max)/2;
      if (data[mindpoint].compareTo(tarjet)==0){
        found=true;
       }else if (data[mindpoint].compare(target)>0)
       if(min<=mindpoint-1){
...
public void insertSort(T[]data){
  for(int index=1; index<data.length;index++){
     T key=data[index];
     int position=index;
     whiles (position>0 && data[position-1].compareTo(key)>0){
...
public void bubleSort(T[]data){
    ...
    //Recorre los datos, efectuando las comparaciones de
    //y examina un valor menos en cada pasada.
